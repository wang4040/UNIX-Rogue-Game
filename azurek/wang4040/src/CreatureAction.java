...
public class CreatureAction extends Action{
    ...
    void Teleport(String name, Creature owner){
		...
		while(true) {
			if( ((Rogue.getDisplayGrid().getTop(newX, newY+Rogue.getTopHeight()).getChar() == '.') || (Rogue.getDisplayGrid().getTop(newX, newY+Rogue.getTopHeight()).getChar() == '#')) && ( (newX != (roomX + owner.getPosX())) || (newY != (roomY + owner.getPosY())) ) ) {
    ...
    void ChangedDisplayedType(String name, Creature owner){
		...
		if(owner.getType() == '@') {
			...
			displayGrid.addObjectToDisplay(new Char(charValue), displayGrid.getPlayerX(), displayGrid.getPlayerY());
			Rogue.displayMessageInfo(message);
		}else {
			int roomX = 0;
			int roomY = 0;
			...
			displayGrid.removeObjectToDisplay(owner.getPosX() + roomX, owner.getPosY() + roomY);
			displayGrid.addObjectToDisplay(new Char(charValue), owner.getPosX() + roomX, owner.getPosY() + roomY);
			Rogue.displayMessageInfo(message);
		}
    }
