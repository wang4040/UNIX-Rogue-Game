...
public class ObjectDisplayGrid extends JFrame implements KeyListener, InputSubject {
	...
	public Char getTop(int x, int y) {
		if ((0 <= x) && (x < objectGrid.length)) {
            if ((0 <= y) && (y < objectGrid[0].length)) {
				if(objectGrid[x][y].empty() == true) {
					return new Char(' ');
	...
	public void removeDontPrint(int x, int y) {
		if ((0 <= x) && (x < objectGrid.length)) {
            if ((0 <= y) && (y < objectGrid[0].length)) {
                objectGrid[x][y].pop();
                if (objectGrid[x][y].empty() == true){
	...
	public void refresh() {
		int x, y;
		for (x = 0; x < objectGrid.length; x++) {
            for (y = 0; y < objectGrid[0].length; y++) {
				if (objectGrid[x][y].empty() == false){
					writeToTerminal(x, y);
				}
            }
        }
