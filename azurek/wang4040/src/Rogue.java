...
public class Rogue implements Runnable{
	...
	public static boolean CombatSimulator(int x, int y){
		...
		monsters.get(monsterSpot).doHitAction();
	...
	public static void displayMessagePack(String msg){
		...
		while (displayGrid.getObjectGrid()[i][gameHeight + topHeight + bottomHeight - 3].empty() == false){
			displayGrid.clearObjectDisplay(i, gameHeight + topHeight + bottomHeight - 3);
			i++;
		}
	}
	...
	public static void displayInventory(){
		...
		for (i = 0; i < pack.size() - 1; i++){
			...
			if ((pack.get(i).getType() == ')') || pack.get(i).getType() == ']'){
				msg += " STR " + pack.get(i).getIntValue();
			}
			msg += ", ";
		}
		msg += (char) (i + '0') + ":";
	...
	public static void hallucinateStep(){
		...
		for(x = 0; x < width; x++) {
			for(y = topHeight; y < topHeight + gameHeight; y++) {
				if(displayGrid.getTop(x, y).getChar() != ' ') {
					c = getRandomChar();
					while(displayGrid.getTop(x, y).getChar() == c.getChar()) {
						c = getRandomChar();
					}
					displayGrid.addObjectToDisplay(c, x, y);
					displayGrid.removeDontPrint(x, y);
				}
			}
		}
	}
